...
export default function Footer() {
  ...
  return (
    <footer className="w-full bg-[#F7F7F7] border-t border-[#DDDDDD] mt-auto">
      <div className="max-w-[1700px] mx-auto px-4 sm:px-8 md:px-10 lg:px-12 py-10">
        
        {/* Social Icons */}
        <div className="flex justify-center items-center gap-6 mb-4 text-zinc-700">
          <button
            type="button"
            onClick={() => handleSocialClick('Facebook')}
            className="hover:text-[#dc3545] transition-colors p-2 rounded-full hover:bg-white shadow-xs cursor-pointer border-none bg-transparent"
            aria-label="Facebook"
          >
        ...
        {/* Copyright */}
        <div className="text-center text-xs font-semibold text-zinc-600 mb-4 flex items-center justify-center gap-1">
          <span>&copy; {new Date().getFullYear()} Vistaro, Inc. Crafted with</span>
          <Heart className="w-3.5 h-3.5 fill-[#dc3545] text-[#dc3545]" />
      ...
      {modalApp && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 text-center shadow-2xl border border-zinc-200 space-y-4">
            
            {/* Header Icon */}
            <div className="w-14 h-14 rounded-2xl bg-red-50 text-[#dc3545] mx-auto flex items-center justify-center shadow-xs">
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full bg-[#dc3545] hover:bg-[#b02a37] text-white text-xs font-bold py-3 px-6 rounded-full transition-all shadow-xs cursor-pointer"
              >
                Got it, thank you!
              </button>
            </div>
