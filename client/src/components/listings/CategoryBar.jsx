    ...
    <div className="w-full flex items-center justify-between gap-3 py-3 mb-6 border-b border-vistaro-border transition-colors duration-200">
      ...
      <div className="relative md:hidden" ref={mobileCatRef}>
        <button
          ...
          className={`flex items-center gap-2 px-3.5 py-2 rounded-full border text-xs font-bold transition-all shadow-xs cursor-pointer ${isMobileCategoryOpen || selectedCategory !== 'All'
        ...
        {isMobileCategoryOpen && (
          <div className="absolute left-0 top-full mt-2 w-72 max-w-[85vw] bg-vistaro-surface/95 backdrop-blur-md rounded-3xl shadow-2xl border border-vistaro-border p-2 z-50 animate-in fade-in zoom-in-95 duration-200">
            <div className="px-3 py-2 border-b border-vistaro-border mb-1 flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase tracking-wider text-vistaro-muted">
                Explore Categories
              </span>
              <span className="text-[10px] text-vistaro-muted font-semibold">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-2xl transition-all text-left cursor-pointer ${isSelected
                        ? 'bg-vistaro-secondary text-vistaro-accent font-bold'
                        : 'hover:bg-vistaro-secondary text-vistaro-secondary hover:text-vistaro-primary font-medium'
                      }`}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-vistaro-muted font-normal">
                          {cat.description}
                        </div>
                      </div>
                    </div>
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-1.5 pb-2 shrink-0 border-b-2 transition-all cursor-pointer group ${isSelected
                  ? 'border-vistaro-accent text-vistaro-accent opacity-100 font-semibold'
                  : 'border-transparent text-vistaro-muted hover:text-vistaro-primary hover:border-vistaro-border opacity-80 hover:opacity-100 font-medium'
                }`}
            >
              <Icon
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        {onOpenFilterModal && (
          <button
            ...
            className="flex items-center gap-1.5 sm:gap-2 border border-vistaro-border hover:border-vistaro-muted rounded-full px-3 sm:px-3.5 py-2 text-xs font-semibold text-vistaro-primary hover:shadow-xs transition-all cursor-pointer bg-vistaro-surface hover:bg-vistaro-secondary"
          >
            <SlidersHorizontal className="w-3.5 h-3.5 text-vistaro-secondary" />
            <span className="hidden xs:inline">Filters</span>
            {activeFilterCount > 0 && (
              <span className="w-4 h-4 rounded-full bg-vistaro-accent text-white text-[10px] flex items-center justify-center font-bold">
                {activeFilterCount}
              </span>
            )}
          </button>
        ...
        {onSortChange && (
          <div className="relative" ref={sortRef}>
            <button
              ...
              className="flex items-center gap-1.5 border border-vistaro-border hover:border-vistaro-muted rounded-full px-3 sm:px-3.5 py-2 text-xs font-semibold text-vistaro-primary hover:shadow-xs transition-all cursor-pointer bg-vistaro-surface hover:bg-vistaro-secondary"
            >
            ...
            {isSortOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-vistaro-surface rounded-2xl shadow-xl border border-vistaro-border py-2 z-50 animate-fade-in divide-y divide-vistaro-border">
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-xs hover:bg-vistaro-secondary transition-colors cursor-pointer ${sortOption === '' ? 'font-bold text-vistaro-accent bg-vistaro-secondary' : 'text-vistaro-primary'
                    }`}
                >
                  Featured
                </button>
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-xs hover:bg-vistaro-secondary transition-colors cursor-pointer ${sortOption === 'price_asc' ? 'font-bold text-vistaro-accent bg-vistaro-secondary' : 'text-vistaro-primary'
                    }`}
                >
                  Price: Low to High
                </button>
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-xs hover:bg-vistaro-secondary transition-colors cursor-pointer ${sortOption === 'price_desc' ? 'font-bold text-vistaro-accent bg-vistaro-secondary' : 'text-vistaro-primary'
                    }`}
                >
                  Price: High to Low
                </button>
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-xs hover:bg-vistaro-secondary transition-colors cursor-pointer ${sortOption === 'newest' ? 'font-bold text-vistaro-accent bg-vistaro-secondary' : 'text-vistaro-primary'
                    }`}
                >
                  Newest First
                </button>
