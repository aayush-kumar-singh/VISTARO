...
import TourPackageCard from '../packages/TourPackageCard.jsx';
import { Compass, ArrowRight } from 'lucide-react';
import { useScrollReveal } from '../../hooks/useScrollReveal.js';

export default function LandingFeaturedTours() {
  ...
  useEffect(() => {
    ...
    async function fetchFeaturedTours() {
      try {
        ...
        const data = await tourPackagesApi.getTourPackages({ featured: true, limit: 4 });
        if (isMounted) {
          setTours(data.packages || data.tourPackages || data || []);
        }
      } catch (err) {
        console.error('Failed to load featured tour packages:', err);
        if (isMounted) {
  ...
  // Graceful empty degradation: if 0 featured tour packages, hide section entirely
  if (!loading && (tours.length === 0 || error)) {
    return null;
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 pb-3 border-b border-vistaro-border">
        <div className="space-y-1">
          ...
          <p className="font-sans text-xs sm:text-sm text-vistaro-muted">
            Multi-day guided expeditions, expert local naturalists, and handcrafted itineraries.
          </p>
        </div>
        ...
        >
          <span>All Guided Expeditions</span>
