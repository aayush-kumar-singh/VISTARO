  ...
  return (
    <div className="group bg-vistaro-surface rounded-3xl overflow-hidden border border-vistaro-border hover:border-vistaro-muted hover:shadow-xl transition-all duration-300 flex flex-col h-full relative">
      ...
      <div className="relative aspect-4/3 overflow-hidden bg-vistaro-secondary">
        ...
        <div className="absolute bottom-3 left-3.5 right-3.5 flex items-center justify-between text-white text-body-sm font-semibold drop-shadow-sm">
          <div className="flex items-center gap-1.5">
            <Clock className="w-3.5 h-3.5 text-vistaro-rating" />
            <span>{duration} {duration === 1 ? 'Hour' : 'Hours'}</span>
          </div>

          <span className="text-caption px-2 py-0.5 rounded-full bg-black/50 backdrop-blur-md text-white border border-white/20">
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
          ...
          {Array.isArray(exp.whatsIncluded) && exp.whatsIncluded.length > 0 && (
            <div className="flex flex-wrap gap-1.5 pt-1">
              {exp.whatsIncluded.slice(0, 2).map((inc, i) => (
                <span
                  key={i}
                  className="bg-vistaro-secondary text-vistaro-primary border border-vistaro-border text-2xs font-normal px-2 py-0.5 rounded-md flex items-center gap-1"
                >
        ...
        <div className="pt-3 border-t border-vistaro-border flex items-center justify-between gap-2">
          ...
          <Link
            ...
            className="inline-flex items-center gap-1.5 bg-vistaro-accent hover:bg-vistaro-accent-hover text-white text-cta py-2.5 px-4 rounded-full transition-all shadow-xs cursor-pointer"
          >
            <span>Explore</span>
