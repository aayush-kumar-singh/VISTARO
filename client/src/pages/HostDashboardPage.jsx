  ...
  if (!user) {
    return (
      <div className="max-w-md mx-auto my-16 p-8 bg-vistaro-surface border border-vistaro-border rounded-3xl text-center space-y-4 shadow-sm text-vistaro-primary">
        <LayoutDashboard className="w-10 h-10 text-vistaro-accent mx-auto" />
        <h2 className="text-xl font-bold text-vistaro-primary">Host Dashboard</h2>
        <p className="text-sm text-vistaro-muted">Sign in to view your properties, track earnings, and manage reservations.</p>
        <Link
          to="/login"
  ...
  if (error) {
    return (
      <div className="max-w-md mx-auto my-16 p-8 bg-vistaro-surface border border-vistaro-error/30 rounded-3xl text-center space-y-4 shadow-sm text-vistaro-primary">
        ...
        <h2 className="text-xl font-bold text-vistaro-primary">Dashboard Failed to Load</h2>
  ...
  return (
    <div className="w-full space-y-8 text-vistaro-primary transition-colors duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-vistaro-border">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-vistaro-primary tracking-tight flex items-center gap-2">
        ...
        <Link
          to="/listings/new"
          className="inline-flex items-center gap-2 bg-vistaro-surface border border-vistaro-border hover:bg-vistaro-secondary text-vistaro-primary text-xs sm:text-sm font-bold py-3 px-6 rounded-full transition-all shadow-xs shrink-0 cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-vistaro-surface p-5 rounded-3xl border border-vistaro-border shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-vistaro-muted">Properties</span>
            <h3 className="text-2xl font-extrabold text-vistaro-primary mt-1">{metrics.totalProperties}</h3>
            <span className="text-xs text-vistaro-muted mt-0.5 block">Active published stays</span>
          </div>
        ...
        <div className="bg-vistaro-surface p-5 rounded-3xl border border-vistaro-border shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-vistaro-muted">Gross Earnings</span>
            <h3 className="text-2xl font-extrabold text-vistaro-primary mt-1">{formatPrice(metrics.totalRevenue)}</h3>
            <span className="text-xs text-vistaro-success font-semibold mt-0.5 block">From confirmed trips</span>
          </div>
        ...
        <div className="bg-vistaro-surface p-5 rounded-3xl border border-vistaro-border shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-vistaro-muted">Upcoming Guests</span>
            <h3 className="text-2xl font-extrabold text-vistaro-primary mt-1">{metrics.totalGuests}</h3>
            <span className="text-xs text-vistaro-muted mt-0.5 block">{dashboard?.upcomingBookings?.length || 0} upcoming stay(s)</span>
          </div>
          <div className="w-12 h-12 rounded-2xl bg-vistaro-secondary text-vistaro-accent border border-vistaro-border flex items-center justify-center">
        ...
        <div className="bg-vistaro-surface p-5 rounded-3xl border border-vistaro-border shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-vistaro-muted">30-Day Occupancy</span>
            <h3 className="text-2xl font-extrabold text-vistaro-primary mt-1">{metrics.occupancyRate}%</h3>
            <span className="text-xs text-vistaro-muted mt-0.5 block">Across all your listings</span>
          </div>
      ...
      <div className="bg-vistaro-surface rounded-3xl p-6 border border-vistaro-border shadow-xs space-y-4">
        <h3 className="text-lg font-bold text-vistaro-primary">Listing Performance</h3>

        {listingStats.length === 0 ? (
          <p className="text-xs text-vistaro-muted py-4 text-center">No properties listed yet.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-vistaro-border text-vistaro-muted uppercase text-[10px] font-bold tracking-wider">
                <tr>
              ...
              <tbody className="divide-y divide-vistaro-border font-medium text-vistaro-secondary">
                {listingStats.map((stat) => (
                  <tr key={stat.listing._id} className="hover:bg-vistaro-secondary/50 transition-colors">
                    <td className="py-3.5 pr-3 font-bold text-vistaro-primary max-w-xs truncate">
                      {stat.listing.title}
                    </td>
                    <td className="py-3.5 pr-3 text-vistaro-muted">{stat.listing.location}</td>
                    ...
                    <td className="py-3.5 pr-3 font-bold text-vistaro-success">{formatPrice(stat.revenue)}</td>
                    <td className="py-3.5 text-right space-x-2">
                      <Link
                        to={`/listings/${stat.listing._id}`}
                        className="text-vistaro-accent hover:underline font-semibold"
                      >
                        View
                      </Link>
                      <Link
                        to={`/listings/${stat.listing._id}/edit`}
                        className="text-vistaro-secondary hover:text-vistaro-primary hover:underline font-semibold"
                      >
                        Edit
                      </Link>
                    </td>
      ...
      <div className="bg-vistaro-surface rounded-3xl p-6 border border-vistaro-border shadow-xs space-y-4">
        <h3 className="text-lg font-bold text-vistaro-primary">Recent Guest Reservations</h3>

        {incomingBookings.length === 0 ? (
          <p className="text-xs text-vistaro-muted py-4 text-center">No guest reservations recorded yet.</p>
        ) : (
          <div className="divide-y divide-vistaro-border">
            {incomingBookings.map((b) => (
              <div key={b._id} className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h4 className="font-bold text-sm text-vistaro-primary">{b.listing?.title || 'Listing'}</h4>
                ...
                <div className="flex items-center gap-4">
                  <span className="font-bold text-sm text-vistaro-primary">{formatPrice(b.totalPrice)}</span>
                  <span
                    className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border border-vistaro-border ${
